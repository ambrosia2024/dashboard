    ...
    try:
        with PiSSHClient() as client:
            ...
            if not remote_folder.strip():
                ...
                # Disallow absolute targets for /transload to prevent writes outside the pool
                return JSONResponse(
                    status_code=400,
                    content={"status": "error",
                             "message": "Absolute paths are not allowed for transload; provide a relative subfolder under 'transload'."}
                )
                # # 2. Absolute path
                # target_dir = remote_folder.rstrip("/")
                ...
                subpath = posixpath.join("transload", raw)
                if subpath.startswith("/") or ".." in subpath:
