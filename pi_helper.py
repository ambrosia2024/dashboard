    ...
    def list_files(self, remote_folder=None):
        ...
        if err:
            if "No such file or directory" in err:
                msg = f"[Info] Remote folder '{target_dir}' does not exist."
                print(msg)
                return {"status": "not_found", "path": target_dir, "items": []}
            else:
                print(f"[Error] Failed to list files in '{target_dir}':\n{err}")
                return {"status": "error", "path": target_dir, "error": err}
        elif out:
            print(f"[Contents of '{target_dir}']:\n{out}")
            return {"status": "ok", "path": target_dir, "raw": out}
        else:
            msg = f"[Info] Folder '{target_dir}' is empty."
            print(msg)
